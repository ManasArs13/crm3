<x-app-layout>
    ...
    <div class="w-11/12 max-w-7xl mx-auto py-8">
        ...
        @if (isset($entity) && $entity != '')
            <h3 class="text-4xl font-bold mb-6">{{ $entity }} №{{ $outgoing->id }}</h3>
        @endif
        ...
        <div
            ...
            <div class="flex flex-col w-100 p-1 bg-white overflow-x-auto">
                <form action="{{ route($action, ['outgoing' => $outgoing->id]) }}" method="post">
                    @csrf
                    @method("PATCH")

                    <div class="min-h-6 px-5 py-1">

                        <div class="flex flex-row basis-full justify-end my-2">
                            <span class="font-light text-sm">обновлено: {{ $outgoing->updated_at }}</span>
                        </div>
                        ...
                        <div x-data="products">
                                ...
                                <div class="flex flex-row mb-1 w-full">
                                    ...
                                    <button @click="removeRow(row)" type="button"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                            data-slot="icon" class="w-6 h-6 m-auto fill-gray-400 hover:fill-red-500">
                            ...
                            <div class="flex flex-row mb-1 w-full rounded p-2">
                                <div class="flex basis-8/12">
                                    <textarea name="description" class="w-full rounded border-neutral-200" placeholder="Комментарий">{{ $outgoing->description }}</textarea>
                                </div>
                        ...
                        <script>
                            document.addEventListener('alpine:init', () => {
                                Alpine.data('products', () => ({
                                    init() {
                                        positions = {!! $outgoing->products !!};
                                        ents = {!! $products !!};
                                        this.entities = ents;
                                        console.log({!! $outgoing->products !!})
                                        let row_i = 0;

                                        if (positions.length > 0) {
                                            this.rows = positions.map(pos => ({
                                                id: row_i++,
                                                product: pos.pivot.product_id,
                                                count: +pos.pivot.quantity,
                                                price: +pos.price,
                                                sum: pos.price * pos.pivot.quantity
                                            }));
