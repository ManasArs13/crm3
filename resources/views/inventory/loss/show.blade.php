<x-app-layout>
    ...
    @if (isset($entity) && $entity != '')
        <x-slot:title>
            {{ __('entity.' . $entity) }} {{ $loss->name }}
        </x-slot>
    @endif

    <div class="w-11/12 mx-auto py-8">
        @if (isset($entity) && $entity != '')
            <h3 class="text-4xl font-bold mb-6">{{ __('entity.' . $entity) }} {{ $loss->name }}</h3>
        @endif

        <div
            class="block rounded-lg bg-white text-closs shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04) mb-8">

            {{-- header --}}
            <div class="border-b-2 border-neutral-100">
                <div class="flex flex-row w-full p-3 justify-between">
                    <h5> от {{ $loss->moment }}</h5>
                </div>
            </div>
        ...
        <div
            class="block rounded-lg bg-white text-closs shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)">
            ...
            <div class="flex flex-col w-100 p-1 bg-white overflow-x-auto">
                <table class="text-left text-nowrap">
                    ...
                    <tbody>
                        @foreach ($loss->positions as $product)
                            <tr class="border-b-2">
