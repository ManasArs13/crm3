...
use App\Models\EnterPosition;
use App\Models\Product;
use App\Services\Api\MoySkladService;

class EnterService implements EntityInterface
{
    ...
    public function import(array $rows)
    {
        foreach ($rows["rows"] as $row) {
            $entity = Enter::firstOrNew(['ms_id' => $row['id']]);
            ...
            if (isset($row["positions"])) {
                ...
                foreach ($products as $product) {
                    $entity_product = EnterPosition::firstOrNew(['ms_id' => $product['id']]);

                    if ($entity_product->ms_id === null) {
                        $entity_product->ms_id = $product['id'];
                    }

                    $entity_product->enter_id = $entity->id;
